...
function parseMarkdownLink(token: string): { label: string; href: string } | null {
...
function renderInline(text: string): React.ReactNode {
  return splitInlineTokens(text).map((token, idx) => {
    ...
    if (token.type === "link") {
      const link = parseMarkdownLink(token.value) ?? { label: token.value, href: "#" };
...
function renderHeading(trimmed: string, level: number, key: number): React.ReactNode {
...
function renderUnorderedList(items: string[], baseKey: number): React.ReactNode {
  return (
    <ul
      key={`ul-${baseKey}`}
...
function collectBulletItems(lines: string[], start: number): { items: string[]; end: number } {
  const items: string[] = [];
  let i = start;
...
function collectOrderedItems(lines: string[], start: number): { items: string[]; end: number } {
  const items: string[] = [];
  let i = start;
...
function parseLine(lines: string[], i: number, elements: React.ReactNode[]): number {
  const line = lines[i];
